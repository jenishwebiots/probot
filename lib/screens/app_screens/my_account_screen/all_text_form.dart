    ...
    return GetBuilder<MyAccountController>(builder: (myAccountCtrl) {
      return StreamBuilder(
        stream: FirebaseFirestore.instance
            .collection('users')
            .where('id', isEqualTo: myAccountCtrl.id).limit(1).snapshots(),
        builder: (context,snapshot) {
          if (snapshot.hasData){
            myAccountCtrl.firstNameController.text = snapshot.data!.docs[0].data()["nickname"] ??"";
            myAccountCtrl.emailController.text = snapshot.data!.docs[0].data()["email"] ?? "";
            myAccountCtrl.numberController.text = snapshot.data!.docs[0].data()["phone"] ?? "";
            return SizedBox(
                child:
