    ...
    return GetBuilder<OnBoardingController>(builder: (_) {
      return WillPopScope(
          ...
          child: Scaffold(
              ...
              body: StreamBuilder(
                stream: FirebaseFirestore.instance.collection("onBoardScreen").snapshots(),
                builder: (context,snapShot) {
                  if(snapShot.hasData) {
                    List images = snapShot.data!.docs[0].data()["images"];
                    List languages = snapShot.data!.docs[0].data()["languages"];
                    log("========+++++=======${appCtrl.languageVal}");
                    int i = languages.indexWhere((element) => element["language"] == appCtrl.languageVal);
                    log("===============$i");
